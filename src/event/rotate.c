...
/*   Updated: 2024/01/26 17:05:39 by jcoquard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_left(t_cub_entity *entity)
{
	...
	entity->dir.y = old_dir.x * sin(-SPEED_ROTATE) + entity->dir.y * cos(-SPEED_ROTATE);
	old_plan.x = entity->plan.x;
	entity->plan.x = entity->plan.x * cos(-SPEED_ROTATE) - entity->plan.y * sin(-SPEED_ROTATE);
	entity->plan.y = old_plan.x * sin(-SPEED_ROTATE) + entity->plan.y * cos(-SPEED_ROTATE);
